...
signing key, converts it with `gpg --dearmor` into
...
5. The CI workflow runs the playbook directly on the GitHub Actions runner so every role is executed during automated tests.
